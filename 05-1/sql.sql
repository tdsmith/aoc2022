CREATE MACRO list_reverse(l) AS (
    SELECT list(col ORDER BY idx DESC)
    FROM (SELECT generate_subscripts(l, 1) AS idx, UNNEST(l) AS col)
);
    ...
    , transfer AS (
        SELECT
            ...
            CASE
                ...
                WHEN idx = dest THEN list_cat(
                    stack,
                    list_reverse(stacks[source][len(stacks[source])-n+1:])
                )
                ELSE stack
            END AS stack
        FROM unpacked
